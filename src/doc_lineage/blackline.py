...
def align_sections(
    ...
    *,
    ...
    left_by_id = {section.section_id: section for section in doc_a.sections}
    right_by_id = {section.section_id: section for section in doc_b.sections}
    ordered_ids = sorted(set(left_by_id) | set(right_by_id), key=lambda value: int(value))
    ...
    for section_id in ordered_ids:
        left = left_by_id.get(section_id)
        right = right_by_id.get(section_id)
        left_conf = left.header_confidence if left is not None else 1.0
        right_conf = right.header_confidence if right is not None else 1.0
